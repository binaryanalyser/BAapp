...
const LiveTicks: React.FC<LiveTicksProps> = ({ symbols }) => {
  const { ticks, subscribeTo, unsubscribeFrom, isConnected: wsContextConnected } = useWebSocket();
  ...
  // Update connection status based on WebSocket context
  useEffect(() => {
    if (wsContextConnected) {
  ...
  // Handle tick data from WebSocket context
  useEffect(() => {
    const tickData = ticks[selectedSymbol];
    if (tickData) {
      console.log('Received tick for', selectedSymbol, ':', tickData.price);
      
      const newTickData: TickData = {
        id: tickData.id || '',
        symbol: selectedSymbol,
        price: tickData.price,
        epoch: tickData.epoch || Date.now() / 1000,
      ...
      const newDigit = getLastDigit(tickData.price, decimals);
      
      setHasReceivedData(true);
      setDigits(prev => [...prev.slice(1), newDigit]);
      setDigitHistory(prev => [...prev, newDigit].slice(-100)); // Keep last 100 digits
      setPrices(prev => [...prev.slice(-19), tickData.price]);
      setChartData(prev => [...prev.slice(-19), {
        time: (tickData.epoch || Date.now() / 1000) * 1000,
        price: tickData.price,
        timestamp: tickData.epoch || Date.now() / 1000
      }]);
  ...
  }, [selectedSymbol, wsContextConnected, subscribeTo]);
